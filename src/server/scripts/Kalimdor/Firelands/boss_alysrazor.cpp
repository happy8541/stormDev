...
public:
    ...
    {	
		if (Creature* alysrazor = creature->FindNearestCreature(NPC_ALYSRAZOR, 5000.0f))
		{
			...
			player->CastSpell(player,SPELL_FLY,true);	 
		}
		else
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
					...
					case EVENT_INCENDIARY_CLOUD:
						if (phase == PHASE_AIR)
						{
							me->SummonCreature(53541,me->GetPositionX(),me->GetPositionY(),me->GetPositionZ() > 120.0f ? me->GetPositionZ() : 120.0f);
							me->SummonCreature(53554,me->GetPositionX(),me->GetPositionY(),me->GetPositionZ() > 120.0f ? me->GetPositionZ() : 120.0f,me->GetOrientation());
						}
